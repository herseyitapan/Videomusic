...
async def vplay(client, m: Message):
    ...
    if replied:
        if replied.video or replied.document:
            ...
            if len(m.command) < 2:
                ...
                if pq == "720" or "480" or "360":
                    ...
                    Q = 720
                    await loser.edit(
                        "» __only 720, 480, 360 icazə verilir__ \n💡 **hazırda video axışı 720p**"
                    )
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo=f"{IMG_1}",
                    caption=f"💡 **İstədiyiniz sıraya əlavə olundu**\n\n🏷 **ad:** [{songname}]({link})\n💭 **qrup:** `{chat_id}`\n🎧 **Tələb edilən:** {m.from_user.mention()}\n🔢 **At position »** `{pos}`",
            ...
            if len(m.command) < 2:
                ...
                loser = await m.reply("🔎 **axtarılır...**")
                query = m.text.split(None, 1)[1]
                search = ytsearch(query)
                Q = 720
                amaze = HighQualityVideo()
                if search == 0:
                    await loser.edit("❌ **istədiyiniz tapılmadı.**")
                else:
                    songname = search[0]
                    url = search[1]
                    veez, ytlink = await ytdl(url)
                    if veez == 0:
                        await loser.edit(f"❌ yt-dl xətası\n\n» `{ytlink}`")
                    else:
                        if chat_id in QUEUE:
                            pos = add_to_queue(
        ...
        if len(m.command) < 2:
            ...
            amaze = HighQualityVideo()
            if search == 0:
                await loser.edit("❌ **axtardığınız tapılmadı.**")
            else:
                songname = search[0]
                url = search[1]
                veez, ytlink = await ytdl(url)
                if veez == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            ...
                            await m.reply_photo(
                                photo=f"{IMG_2}",
                                caption=f"💡 **video axışı başladı.**\n\n🏷 **ad:** [{songname}]({url})\n💭 **qrup:** `{chat_id}`\n💡 **vəziyyət:** `Oxunur`\n🎧 **Tələb eden:** {m.from_user.mention()}",
                                reply_markup=keyboard,
                            )
                        except Exception as ep:
                            await m.reply_text(f"🚫 Xəta: `{ep}`")
